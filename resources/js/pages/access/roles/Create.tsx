...
export default function RoleCreate({ permissionGroups, userOptions }: Props) {
    ...
    const { data, setData, post, processing } = useForm<{ name: string; permission_ids: number[]; user_ids: number[] }>({
        name: '',
        permission_ids: [],
        user_ids: [],
    });
    ...
    return (
        <PerformancePage title="Create Role" description="Create a role and assign permissions and users." breadcrumbs={breadcrumbs}>
            <form onSubmit={submit} className="w-full bg-slate-50 text-slate-900 font-body min-h-screen relative pb-32">
                ...
                <div className="fixed bottom-0 left-0 lg:left-64 right-0 bg-white/90 backdrop-blur-md border-t border-slate-200/80 p-6 flex justify-end gap-4 shadow-[0_-10px_20px_rgba(0,0,0,0.03)] z-40">
                    <Link href={route('access.roles.index')}>
