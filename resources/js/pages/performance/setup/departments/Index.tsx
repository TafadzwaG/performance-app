...
import { BarChart3, Building2, Eye, Filter, PencilLine, Plus, Search, ShieldAlert } from 'lucide-react';
...
function buildDistribution(items: Department[]) {
...
export default function DepartmentsIndex({ departments, filters, can }: Props) {
    ...
    const inactiveCount = departments.data.filter((department) => !department.is_active).length;
    ...
    const totalOnPage = departments.data.length;
    const totalResults = departments.total ?? totalOnPage;
    const from = departments.from ?? 0;
    const to = departments.to ?? totalOnPage;

    const distribution = useMemo(() => buildDistribution(departments.data), [departments.data]);
    const maxDistribution = Math.max(...distribution.map(([, count]) => count), 1);

    return (
        <PerformancePage
            title="Departments"
            description="Maintain the department catalogue used across employee setup, planning, and reporting."
            ...
            <div className="space-y-8">
                <div className="flex flex-col gap-6">
                    <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
                        <div>
                            <h1 className="text-3xl font-bold tracking-tight text-foreground">Departments Index</h1>
                            <p className="mt-1 text-sm text-muted-foreground">
                                Manage business units, operational codes, and active department availability.
                            </p>
                        </div>

                        {can.create ? (
                            <Button asChild>
                                <Link href={route('performance.setup.departments.create')}>
                ...
                <Card className="overflow-hidden shadow-sm">
                            ...
                            <form onSubmit={submit} className="flex w-full gap-2">
                                <div className="relative flex-1">
                                    ...
                                    <Input
                                        ...
                                        placeholder="Filter by department, code or description..."
                                        className="pl-9"
                                    />
                                </div>
                            ...
                            <tbody className="divide-y">
                                {departments.data.length > 0 ? (
                                    departments.data.map((department) => (
                                        <tr key={department.id} className="group transition-colors hover:bg-muted/20">
                                            ...
                                            <td className="px-6 py-5 text-right">
                                                <div className="flex justify-end gap-2 opacity-100 transition-opacity md:opacity-0 md:group-hover:opacity-100">
                                                    <Button asChild variant="ghost" size="sm">
                                                        <Link href={route('performance.setup.departments.show', department.id)}>
                    ...
                    <div className="flex flex-col gap-4 border-t bg-muted/10 px-6 py-4 md:flex-row md:items-center md:justify-between">
                        <div className="text-xs font-medium text-muted-foreground">
                            Showing {from} to {to} of {totalResults} departments
                        </div>
                ...
                <div className="grid gap-8 lg:grid-cols-2">
                    <Card className="relative overflow-hidden border shadow-sm">
                        <CardContent className="p-8">
                            ...
                            <div className="relative z-10">
                                <h3 className="text-xl font-bold tracking-tight text-foreground">Operating Coverage</h3>
                                <p className="mt-2 text-sm leading-6 text-muted-foreground">
                                    The current page shows {inactiveCount} inactive department{inactiveCount === 1 ? '' : 's'}.
                                    Review archived units regularly to keep employee assignment, planning, and
                                    reporting structures clean.
                                </p>

                                <Button variant="outline" className="mt-6">
                                    <ShieldAlert className="mr-2 h-4 w-4" />
                                    Review Inactive Departments
                                </Button>
                            </div>
                        </CardContent>
                    </Card>
                    ...
                    <Card className="shadow-sm">
                        <div className="border-b px-6 py-5">
                            <h3 className="text-xl tracking-tight text-foreground">Catalogue Distribution</h3>
                            <p className="mt-1 text-sm text-muted-foreground">
                                Top visible department groups by code prefix on this page.
                            </p>
                        </div>
                        ...
                        <CardContent>
                            ...
                            <div className="mt-6 flex items-center gap-3 rounded-lg border bg-muted/10 p-4">
                                ...
                                <div className="text-sm text-muted-foreground">
                                    Prefix counts are inferred from department codes currently visible in the table.
                                </div>
                            </div>
                        </CardContent>
                    </Card>
