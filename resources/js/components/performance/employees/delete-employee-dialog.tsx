...
export default function DeleteEmployeeDialog({ employee, open, onOpenChange }: Props) {
    ...
    useEffect(() => {
        ...
        fetch(route('performance.employees.deletion_impact', { employee_profile: employee.id }), {
            headers: {
    ...
    const submitDelete = (event: FormEvent<HTMLFormElement>) => {
        ...
        router.delete(route('performance.employees.destroy', { employee_profile: employee.id }), {
    ...
    const displayName = employee?.user?.name ?? impact?.employee.name ?? 'Employee';
    const displayEmail = employee?.user?.email ?? impact?.employee.email;
    const employeeNumber = employee?.employee_number ?? impact?.employee.employee_number;
                    ...
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription asChild>
                            <div className="space-y-4 text-left text-sm text-muted-foreground">
                                {employee ? (
                                    <p>
                                        You are about to delete <strong className="text-foreground">{displayName}</strong>
                                        {employeeNumber ? ` (${employeeNumber})` : ''}
                                        {displayEmail ? ` · ${displayEmail}` : ''}. This action cannot be undone.
                                    </p>
                                ) : null}
                                ...
                                <div className="space-y-2">
                                    <Label htmlFor="delete-employee-current-password">Your password</Label>
                                    <PasswordInput
                                        ref={passwordRef}
                                        id="delete-employee-current-password"
