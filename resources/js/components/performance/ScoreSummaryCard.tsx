...
export default function ScoreSummaryCard({ businessScore, valuesScore, overallScore, overallRating }: ScoreSummaryCardProps) {
    ...
    const tone = getPerformanceTone(overallScore);
        ...
        <Card>
            ...
            <CardContent className="grid gap-4 lg:grid-cols-4">
                ...
                <div className="rounded-lg border bg-muted/20 p-4">
                    ...
                    <div className={`inline-flex items-center gap-1 rounded-full border px-2.5 py-1 text-xs font-medium ${tone.badgeClass}`}>
...
function ScoreDonut({ label, value, emphasize = false }: { label: string; value?: number | null; emphasize?: boolean }) {
        ...
        <div className="rounded-lg border bg-muted/20 p-4">
            ...
            <div className="flex items-center gap-3">
                ...
                <div className="space-y-1">
                    <div className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[11px] font-medium ${tone.badgeClass}`}>
                        {value == null ? <CircleDashed className="h-3.5 w-3.5" /> : <CircleGauge className="h-3.5 w-3.5" />}
                        {tone.label}
                    </div>
                </div>
            </div>
